"""
    Implementation of Heterogeneous Earliest Finish Time (HEFT) scheduling algorithm:
    ...
    For inputs HEFT takes a set of tasks, represented as a directed acyclic graph, a set of Processing Elements,
    the times to execute each task on each PE (can be also infity if it can not be mapped on that PE),
    and the times to communicate the results from each job to each of its children between each pair of PEs
    
    Note: for our purposes we assume communication cost is always set to 0.

    This code extends: https://github.com/mackncheesiest/heft
...
def schedule_dag(dag: nx.DiGraph, W: np.array, C: np.array = None, L: np.array = None):
    '''
    ...
    P = W.shape[1]  # Number of PEs
    task_schedules = {}  # Computed schedule
    pes_schedule = {}  #Computed schedule for each PE (a list of task HEFTScheduleEvent)
    ...
    for node in sorted_nodes:
        min_task_schedule = HEFTScheduleEvent(node, -1, np.inf, np.inf)
        min_EDP = np.inf

        # Find the PE that minimize the EFT of the task
        for pe in range(P):
            # TODO: maybe here: if a PE can not run on a specific PE, skip it
            task_schedule = compute_eft(dag, task_schedules, pes_schedule, W, C, L, node, pe)
...
def compute_eft(dag, task_schedules, pes_schedule, W, C, L, node, pe):
    """
    ...
    for prednode in list(dag.predecessors(node)):
        ...
        if C is None or C[predjob.pe, pe] == 0:
            ready_time_t = predjob.end_t
        else:
            ready_time_t = predjob.end_t + dag[predjob.task][node]['weight'] / C[predjob.pe, pe]

            if L is not None:
                ready_time_t = ready_time_t + L[predjob.pe]
    ...
    for idx in range(len(job_list)):
        ...
        if idx == 0:
            if (prev_job.start_t - computation_time) - ready_time > 0:
                ...
                min_schedule = HEFTScheduleEvent(node, pe, job_start, job_start + computation_time)
                break
        ...
        min_schedule = HEFTScheduleEvent(node, pe, ready_time, ready_time + computation_time)
    logger.debug(f"\tFor node {node} on PE {pe}, the EFT is {min_schedule}")
    return min_schedule


def ranku(dag: nx.DiGraph, W: np.array, C: np.array = None, L: np.array = None):
    '''
    ...
    if C is None:
        ...
        diagonal_mask = np.ones(C.shape, dtype=bool)
        np.fill_diagonal(diagonal_mask, 0)
        avg_communication_cost = np.mean(C[diagonal_mask])
        if L is not None:
            avg_communication_cost = avg_communication_cost + np.mean(L)

    # computation matrix
    # Set the average communication cost of edges (eq. 3 in [1])
    # TODO add commcost if needed. For the moment being it is not used
    for edge in dag.edges():
        avg_weight = float(dag.get_edge_data(*edge)['weight']) / avg_communication_cost if C is not None else 0
        logger.debug(
            f"Assigning {edge}'s average weight based on average communication cost. {float(dag.get_edge_data(*edge)['weight'])} => {avg_weight}"
        )
        # nx.set_edge_attributes(dag, {edge: 0}, 'avgweight')
        nx.set_edge_attributes(dag, {edge: avg_weight}, 'avgweight')

    # We are interested in knowing the average execution cost of a task considering all available PEs
    comp_matrix_masked = np.ma.masked_where(W == np.Inf, W)

    # set ranku of exit node: it is equal to the average computation time
    nx.set_node_attributes(dag, {exit_node: np.mean(comp_matrix_masked[exit_node])}, "ranku")
    ...
    while queue:
        ...
        logger.debug(f"\tAssigning rank to {node}: {np.mean(comp_matrix_masked[node])} + {max_successor_ranku}")
        nx.set_node_attributes(dag, {node: np.mean(comp_matrix_masked[node]) + max_successor_ranku}, "ranku")
