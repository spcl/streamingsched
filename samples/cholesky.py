...
'''
     Evaluation over randomly generated Tiled Cholesky Canonical Task graphs.
'''
...
def worker(queue, id, num_pes, exhaustive_results, max_work_results, streaming_interval_results, max_work_gang_results,
    ...
    while (True):
        ...
        exhaustive, max_work, streaming_int, max_work_gang, streaming_int_gang, all_streams, all_non_streams, streaming_slr, non_streaming_slr, sim_error = test_scheduling_heuristics(
            dag, num_pes, source_node, sink_node, n_edges, scheduler, 2, simulate=simulate, run=i)
...
if __name__ == "__main__":
    ...
    for r in range(R):
        ...
        dag, source_node, sink_node = cholesky.build_cholesky_dag(N=N,
                                                                  W=W,
                                                                  random_nodes=True,
                                                                  max_output_volume=W * 1000)

        # Add weights to all nodes
        for n, data in dag.nodes(data=True):
            data['weight'] = W if n != source_node else 0
        ...
        # this_weight = get_average_edge_weight(dag)
        # weight += this_weight
    ...
    if verbose:
        ...
        if num_pes >= dag.number_of_nodes() - 2:
            print(f"streaming depth speedup: {res_all_streams*num_pes:.2f}")
    ...
    results_filename = f'results_cholesky_N_{N}_P_{num_pes}.csv'
    sim_error_filename = f'sim_error_cholesky_N_{N}_P_{num_pes}.csv'
