...
'''
    Evaluation over random synthetic task graphs generated considering
    the chain topology.
'''
...
if __name__ == "__main__":
    ...
    for r in range(R):
        random.seed(r)  # for reproducibility purposes we use the same seed across multiple runs
        dag, source_node, sink_node = chain.build_chain_dag(N=N, W=W, random_nodes=True)

        n_edges = N - 1  # skip root to 1 and N to sink node
    ...
    results_filename = f'results_chain_N_{N}_P_{num_pes}.csv'
    sim_error_filename = f'sim_error_chain_N_{N}_P_{num_pes}.csv'
